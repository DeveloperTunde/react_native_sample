...
const SendWalletPIN = ({route, navigation}) => {

  const {wallet} = route.params;
  
    const {isLoggedIn, isActive, userData,sendamount,userJwt, withdrawalamount} = useSelector(state => state.userReducer);
      ...
      fetch(`${BaseUrl}/pin/validate`, {
        ...
        .then((responseJSON) => {
           ...
           if(responseJSON.status == true && responseJSON.statusCode == 200){
              handleSuccessfullWalletTransfer()
           }else{
             setIsCodeWrong(true);
           }
 ...
 function handleSuccessfullWalletTransfer() {
  const data = {
    "amount": sendamount,
 ...
 fetch(`${BaseUrl}/wallet/crypto-transfer`, {
     method: 'POST', 
     headers: {
       'Accept': 'application/json',
       'Content-Type': 'application/json',
   ...
   .then((responseJSON) => {
      ...
      if(responseJSON.status == true && responseJSON.statusCode == 200){
         navigation.navigate('SendSuccess', {content:` sent to ${wallet.address} Successfully!`})
      }else{
       // setIsCodeWrong(true);
